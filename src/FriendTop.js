    ...
    _proto.loadFriendInfo = function(){
    	...
    	Http.friend("/api/game/loadOtherPlayer",that.id,function(data){
    		...
    		if(!data.success){
    			...
    			that.header.getChildByName("woodValue").text = data.obj.wood;
    			that.header.getChildByName("stoneValue").text = data.obj.stone;
    			that.header.getChildByName("cementValue").text = data.obj.steel;
    			that.header.getChildByName("jewelValue").text = data.obj.money;
