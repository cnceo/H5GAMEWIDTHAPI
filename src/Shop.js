    ...
    _proto.shopOpr = function(event,index){
        ...
        if(event.type == Event.CLICK){
            ...
            if(target.name == "reduceNum"){
                ...
                // console.log(buyNum,id);
                // alert("http://center.atjuqi.com:8088/nccz.html?itemid="+id+"&num="+buyNum+"&token="+this.token)
                window.location = "http://center.atjuqi.com:8088/nccz.html?itemid="+id+"&num="+buyNum+"&token="+localStorage.getItem("access_token");
            }
        }
    }
    //关闭自己
